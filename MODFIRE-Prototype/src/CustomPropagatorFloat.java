...
public class CustomPropagatorFloat extends org.chocosolver.solver.constraints.Propagator<IntVar> {
    ...
    public float gPropagate(int first, int ugIndex, int year, float sum) throws Exception{
    ...
    public float g(int first, int ugIndex, int year, float sum) throws Exception{
        if(ugIndex == first){
    ...
    public float glade(int ugIndex, int period) throws Exception{
    ...
    public ESat isEntailed() { //When it gets here everything is already instantiated, but it doesn't backtrack
        ...
        if (nodes[ugIndex].years[prescIndex][0] > 0) { //if this UG with this PREC has cuts
            ...
            for (int p = 0; p < nodes[ugIndex].years[prescIndex].length; p++) { //for every year that our UG has cuts
                ...
                int year = nodes[ugIndex].years[prescIndex][p]; //current year
                try {
                    float sum = glade(ugIndex, year); //we get the sum
                    if(sum > 50){
                        return ESat.FALSE;
                    }
                } catch (Exception e) {
